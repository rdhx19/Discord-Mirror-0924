...
class Replacement {
  ...
  private createApplyCallback(
    where: ReplacementLocation | undefined
    ...
    switch (where) {
      ...
      case ReplacementLocation.EMBED_COLOR: {
        if (
          !isWildcardRegex(this.replace) &&
          !isValidHexColor(this.replace.source)
        ) {
          throw new Error(
            `Invalid color in your config.yml (only hex is supported). Replace "${this.replace.source}" with a valid hex color (e.g. #3463D9) to fix this error.`
          );
        }
        if (!isValidHexColor(this.with)) {
          throw new Error(
            `Invalid color in your config.yml (only hex is supported). Replace "${this.with}" with a valid hex color (e.g. #3463D9) to fix this error.`
          );
        }
  ...
  private replaceEmbedColor(message: Message): void {
    for (const embed of message.embeds) {
      ...
      if (
        isWildcardRegex(this.replace) ||
        hexColorsAreEqual(embedColor, this.replace.source)
      ) {
...
export class MirrorReplacements {
  ...
  public constructor(replacementsConfig: Record<number, ReplacementConfig>) {
    this.replacements = Object.values(replacementsConfig).map(
      (config) => new Replacement(config)
    );
  }
